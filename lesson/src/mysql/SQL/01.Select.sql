...
# 현재 날짜와 시각
SELECT now();   # 2024-02-01 13:45:21
...
SELECT ROUND(AVG(population)) AS avgPop FROM city 
    WHERE countrycode="KOR";    # 이름을 변경하고 싶을 경우는 AS 입력 (안해도 무방)
...
SELECT r.Continent, r.Name countryName, l.Name cityName, l.Population FROM city l   # r.Continent 에서 r. 은 생략 가능
    JOIN country r 
    ON l.CountryCode=r.Code
